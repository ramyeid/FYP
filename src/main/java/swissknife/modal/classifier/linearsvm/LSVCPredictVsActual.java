package swissknife.modal.classifier.linearsvm;

import swissknife.modal.classifier.RunClassifierPython;

/**
 * Created by ramyeid on 6/10/17.
 */
public class LSVCPredictVsActual extends LinearSVC {


    public LSVCPredictVsActual(){
        super();
        this.action = 1;
    }

    public LSVCPredictVsActual(String inputFile, String keyToPredict, int actionTime) {
        super(inputFile, keyToPredict, actionTime);
        this.action = 1;
    }

    @Override
    public void action() {

        new RunClassifierPython(inputFile,keyToPredict,action,actionTime,"LSVC",actionKeys).run();
    }

}
